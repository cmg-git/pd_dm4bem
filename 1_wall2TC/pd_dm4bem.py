...
def wall2TC(wall_types, walls_def, prefix="w"):
    ...
    def number_branches(wall):
        ...
        """
        # 1 mesh: 2 R and 1 C
        mesh = wall.groupby("ID").agg({"Mesh": "sum"})
        mesh = mesh.squeeze()   # Series or int <-- DataFrame
        if not isinstance(mesh, pd.Series):
            mesh = pd.Series(data=mesh)
            mesh = mesh.rename('Mesh')
...
def file2TC(TC_file, name="w"):
    ...
    w_q = [name + '_q' + str(x) for x in range(A.shape[0])]
    w_θ = [name + '_θ' + str(x) for x in range(A.shape[1])]
...
def bldg2TCd(folder_path):
    """
    Convert a folder containing the files characterizing the building into
    a disassambled thermal circuit. Calls:
        - wall2TC(): from wall_type.csv and wall_data.csv to themal circuit
        - file2TC(): from TCn.csv to thermal circuit.

    Parameters
    ----------
    bldg_path : str
        Path of the folder containing *.csv files describing the disassambled
        thermal cicuit: walls, thermal circuits, and assembly matrix.
    ...
    TCd : dict
        ...
        `wall_data.csv` plus the number of files describing the
        thermal circuits TC.
        ...
        Each circuit is a dictionary:
            ...
            - b: Series, vector of temperture sources on branches;
            - f: Series, vector of flow sources in nodes;
        with indexes `b` for branches, e.g. w0_b1 for wall 0 branch 1,
    ...
    Description of the folder containing the disassembled thermal circuit:
        ...
        - TC*.csv: thermal circuit file containing A, G, C, b, f. see file2TC()

        - wall_types: see wall2TC()
        - walls_def: see wall2TC()
    ...
    wall_files = [wf for wf in csv_files if 'wall' in wf]
    TC_files = [TC_file for TC_file in csv_files if 'TC' in TC_file]

    # Thermal circuits from data & type files of walls
    wall_data = pd.read_csv(wall_files[0])
    wall_type = pd.read_csv(wall_files[1], index_col=0)
    TCd = wall2TC(wall_type, wall_data, name="w")

    # Thermal circuits from files of thermal circuit
    len_TCd_walls = len(TCd)
    for k in range(len(TC_files)):
        TCd[len_TCd_walls + k] = file2TC(TC_files[k], name="c" + str(k))
